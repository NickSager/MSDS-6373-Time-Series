...
plotts.sample.wge(daily_bike_data$Total_Users) 
...
```{r}
# create model 
plotts.sample.wge(daily_bike_data$Total_Users) # looking at the ACF's there seems to be a seasonality of 2
...
plotts.sample.wge(seasonal_gen1)
plotts.sample.wge(seasonal_gen2)
plotts.sample.wge(seasonal_gen3)
plotts.sample.wge(seasonal_gen4) # all these 4 random realizations have the same behavior as the original data so the model seems to be appropriate
...
points(t[725:731], seasonal_stf$f, type="l", lwd=2, lty = 2, col = 'blue')

plot(t[670:731],daily_bike_data$Total_Users[670:731], type = 'l', xlab = "Time", ylab = "Total Users")
points(t[672:731], seasonal_ltf$f, type="l", lwd=2, lty = 2,col = 'blue')
...
# Rolling Window RMSE
seasonal_strwRMSE = roll.win.rmse.wge(daily_bike_data$Total_Users, horizon = 7, phi = seasonal_est$phi, theta = seasonal_est$theta, s = 2)
seasonal_ltrwRMSE = roll.win.rmse.wge(daily_bike_data$Total_Users, horizon = 60, phi = seasonal_est$phi, theta = seasonal_est$theta, s = 2)
...
mlr_st_ASE= mean((daily_bike_data$Total_Users[725:731] - mlr_pred$pred)^2)
