...
# TODO: Fill in model metrics
# Create a data frame with the metrics
model_metrics <- data.frame(
  Model = c("ARMA", "ARIMA", "VAR", "MLR", "MLP", "Ensemble"),
  ASE = c(ase_model1, ase_model2, ase_model3),
  rwRMSE = c(rwrmse_model1, rwrmse_model2, rwrmse_model3)
)

# Apply conditional formatting to the ASE column
model_metrics$ASE <- cell_spec(
  model_metrics$ASE, 
  color = ifelse(model_metrics$ASE == min(model_metrics$ASE), "red", "black")
)

# Create and format the table
kable(model_metrics, format = "html", escape = F, caption = "Model Performance Metrics") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F) %>%
  column_spec(1, bold = T) %>%
  column_spec(2:3, format = "html", digits = 2)
